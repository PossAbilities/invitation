# PossAbilities Invitations

A hosted PossAbilities invitation prototype for creating an event campaign,
previewing the email envelope, and showing an animated RSVP invitation page.

## Current Prototype

- Campaign command centre for starting a new invitation or returning to active
  events without digging through crowded panels.
- Clear dashboard sections for starting, continuing, and spotting items that
  need attention.
- Guided invitation builder with clear design, recipients, and review steps.
- Responsive layouts for desktop, tablet, and mobile screens.
- Editable event name, date, time, location, host, message, and card accent.
- Animated recipient preview with a closed envelope that opens into a full card
  view with RSVP buttons.
- Guest actions for downloading an add-to-calendar file and opening the event
  location in maps.
- Dedicated recipient manager with one-by-one entry plus CSV and pasted
  spreadsheet row import.
- Separate superuser monitoring area for registration, approvals, email queue,
  data requests, and platform health.
- Guest list, campaign response summary, and site-specific social preview
  metadata.

## Next Product Steps

- Add user accounts and role-based access.
- Store events, guests, invite links, RSVP responses, consent, and audit logs.
- Connect a transactional email provider for real sending, bounce handling, and
  unsubscribe support.
- Add privacy, retention, export, and delete tools before storing real personal
  data.

## Local Commands

```bash
npm run dev
npm run build
npm test
```
